...
import be.develdploeters.service.AddressService;
...
/**
 * REST controller for managing {@link be.develdploeters.domain.Address}.
 */
@RestController
@RequestMapping("/api")
public class AddressResource {
    ...
    /**
     * {@code GET  /addresses/currentUser} : get the current user address.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the addressDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/addresses/currentUser")
    public ResponseEntity<AddressDTO> getCurrentUserAddress() {
        log.debug("REST request to get Address");
        AddressDTO addressDTO = addressService.findCurrentUser();

        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, "Current user address"))
            .body(addressDTO);
    }
